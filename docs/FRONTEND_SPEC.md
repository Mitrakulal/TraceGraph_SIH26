...
`MARK REVIEWED` and `ESCALATE` are API-backed reviewer decisions. The frontend sends the action to `POST /api/v1/alerts/:alertId/reviews`, then renders the returned `review_state`. The frontend must not locally invent or persist a review decision.

`EXPORT REPORT` is a frontend formatting action only. It may export the currently visible, API-returned synthetic alert fields and the mandatory synthetic-data limitation; it must not add model values or hidden truth fields.
...
Before the local API is available, the frontend may use only API-shaped safe fixtures generated by the backend data owner in `packages/fixtures/`. It must not invent local alert rows, scores, counts, model evidence, or reviewer states in page components.
...
| Alert search | Filter synthetic alerts returned by the API or an approved safe fixture |
| Alert filters | Filter API/approved-fixture data; never change or recalculate backend values |
| Investigate | Open investigation |
| Mark Reviewed | Call the review API with `decision: "REVIEWED"`; replace the UI state with the returned alert state |
| Escalate | Call the review API with `decision: "ESCALATED"`; replace the UI state with the returned alert state |
